...
contract DisputeResolutionCenter{
    ...
    constructor (address _owner, address _consumerDashboardGen, address _protocolWalletGen) {
    ...
    function addDisputeToCenter(address _sc, address _owner, address _to, uint _amount, string memory _output, uint _id, uint _cid, bool _win, bool _lose, bool _undecided) public {
        disputesLength ++ ;
        disputes.push(Dispute(_sc, _owner, _to, _amount, _output, _id, _cid, Decision(_win, _lose, _undecided), new address[](0)));  
    }

    function updateDisputeResult(bool _win, bool _lose, uint _cid) public {
        require(disputes[_cid].decision.undecided == true, "dispute already resolved");
        ...
        if (_win == true && _lose == false){
            ...
            wallet.receiveChargeback(disputes[_cid].amount, payable(disputes[_cid].owner), disputes[_cid].output);
        }
    }
    ...
    function createJurorWallet() external {
        require(jurors[msg.sender] == address(0), "You have already created a staking wallet");
